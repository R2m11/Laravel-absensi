...
<div class="card mx-5 my-4">
    <div class="card-header py-3">
        ...
        <form action="/userbagian/{{$userbagian->id}}" method="POST">
            @csrf
            @method('put')
            <div class="form-group">
                <label for="bagian_id">Bagian:</label>
                <select name="bagian_id" id="bagian_id" class="form-control" required>
                    <option value="" selected disabled>{{ $userbagian->bagian->nama_bagian }}</option>
                    @foreach($bagian as $b)
            ...
            <div class="form-group">
                ...
                <input type="text" id="nama_user_bagian" class="form-control" name="nama_user_bagian" value="{{old('nama_user_bagian',$userbagian->nama_user_bagian)}}" required>
            </div>
